...
typedef struct {
	...
	pthread_cond_t flag;	//conditional variable for synchronization
...
my_semaphore *my_semaphore_init(){
	...
	s->my_value = 1;
	//initiliaze mutex and conditional variable
	pthread_mutex_init(&(s->lock), NULL);
	pthread_cond_init(&(s->flag), NULL);
	//return newly created my_semaphore object pointer
	return s;
}
...
void wait(my_semaphore *s){
	while(1){
    	...
    	if(s->my_value <= 0) {
    		//block other threads
    		pthread_cond_wait(&(s->flag),&(s->lock));
    		//release mutex
